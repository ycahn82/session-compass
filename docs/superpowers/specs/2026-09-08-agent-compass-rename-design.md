# Agent Compass 이름 변경 설계

## 1. 목표

현재 `cli-sessions` 포크를 공개적으로 배포 가능한 별도 프로젝트인 `agent-compass`로 정리한다.

최종 공개 이름은 다음과 같다.

| 구분 | 최종 이름 |
| --- | --- |
| PyPI distribution | `agent-compass` |
| Python import package | `agent_compass` |
| CLI command | `acompass` |
| GitHub repository | `ycahn82/agent-compass` |
...
Agent Compass는 Claude Code, Codex, Antigravity CLI, Copilot CLI가 로컬에 저장한 session을 한곳에서 발견하고 식별하며 재개하기 위한 local-first CLI다.
...
- `[project.scripts]` entry point를 `acompass = "agent_compass.cli:main"`으로 변경한다.
- 소스 패키지 디렉터리를 `src/cli_sessions`에서 `src/agent_compass`로 변경한다.
- Python 모듈의 내부 import와 모듈 docstring을 `agent_compass`에 맞춘다.
...
GitHub repository 이름을 `ycahn82/cli-sessions`에서 `ycahn82/agent-compass`로 변경한다.
...
1. 프로젝트 제목과 PyPI badge를 `agent-compass`로 변경
2. `acompass` 설치 및 사용 예시
...
9. 서비스 CLI가 독립적으로 업데이트될 수 있으므로 `agent-compass`가 지원하는 compatibility floor와 weekly CI monitoring을 운영한다는 설명
...
README의 모든 사용 예시는 `sessions`가 아니라 `acompass`를 사용한다.

### 3.4 Master plan과 문서

기존 `cli-sessions-customization-master-plan.md`의 제품명을 `Agent Compass`로 갱신하고, 명령 예시는 현재 구현 범위에 맞게 `acompass`로 갱신한다. 원본 upstream을 가리키는 문맥에서는 `cli-sessions`를 유지한다.
...
새 설치:   pip install agent-compass -> acompass
```

따라서 기존 사용자의 `sessions` 명령을 가로채거나 덮어쓰지 않는다. 새 프로젝트에서 `sessions` alias를 제공하지 않는 것은 의도적인 충돌 방지 정책이다.

새 package의 import namespace도 `agent_compass`로 변경하여, 같은 Python environment에서 두 distribution을 설치할 때 `cli_sessions` 파일이 서로 덮어쓰이는 위험을 피한다. 다만 CLI 도구는 `pipx` 또는 `uv tool`처럼 별도 environment에 설치하는 것을 README에서 권장한다.

## 5. 구조와 책임 경계

이름 변경은 기능 동작을 바꾸지 않는 packaging/refactor 단계로 유지한다.

- `agent_compass/cli.py`: argparse, 목록 출력, 선택, resume orchestration
- `agent_compass/agents/`: Claude, Codex, Antigravity, Copilot별 storage/metadata/resume adapter
- `agent_compass/compatibility.py`와 관련 workflow: 서비스 CLI compatibility contract
...
2. `python -m agent_compass.cli --help`가 실행된다.
3. help 출력에 `acompass` 사용법과 `-d`/`--dangerously-skip-permissions`가 나타난다.
4. `python -m cli_sessions.cli`가 더 이상 프로젝트 내부의 정상적인 public module 경로가 아님을 확인한다.
5. wheel에 `agent_compass/`가 포함되고 `cli_sessions/`가 포함되지 않는다.
6. 설치 후 `acompass --claude`와 provider filter가 동작한다.
7. `sessions` command는 새 package 설치만으로 생성되지 않는다.
8. README의 install, usage, source, PyPI 링크가 모두 새 이름을 가리킨다.
9. master plan의 사용자-facing 명령 예시가 `acompass`와 일치한다.
10. repository rename 후 `origin`은 `agent-compass`, `upstream`은 원본 `cli-sessions`를 가리킨다.
